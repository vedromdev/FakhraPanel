...
# Railway/PaaS detection: containers there run without NET_ADMIN/iptables, so
# fail2ban cannot work and its startup failure would kill the panel. Disable it
# automatically unless the operator forced it on explicitly.
if [ "$XUI_ENABLE_FAIL2BAN" = "true" ]; then
    if ! command -v iptables >/dev/null 2>&1; then
        echo "[entrypoint] iptables not available - disabling fail2ban (managed hosting/Railway)."
        echo "[entrypoint] Set XUI_ENABLE_FAIL2BAN=false in your environment to silence this."
        XUI_ENABLE_FAIL2BAN="false"
    elif ! iptables -L -n >/dev/null 2>&1; then
        echo "[entrypoint] no NET_ADMIN capability - disabling fail2ban (managed hosting/Railway)."
        echo "[entrypoint] Set XUI_ENABLE_FAIL2BAN=false in your environment to silence this."
        XUI_ENABLE_FAIL2BAN="false"
    fi
fi
